...
export const generateMobileShopHTML = (data: TemplateData, projectName: string): string => {
  ...
  const productCardsHTML = products.map((product) => `
    <div class="flex flex-col gap-3 pb-3 bg-white rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300">
      <div class="relative w-full aspect-square overflow-hidden rounded-t-xl">
        ...
        <div class="absolute top-3 right-3 bg-[#FF4500] text-white text-xs font-bold px-2 py-1 rounded-full">
          ${getText(product.discountId, product.defaultDiscount)}
        </div>
      </div>
      <div class="px-4 flex flex-col gap-3">
        <div class="flex flex-col">
          <p class="text-base font-bold leading-normal">${getText(product.nameId, product.defaultName)}</p>
          <p class="text-lg font-black text-[#FF4500] leading-normal">${getText(product.priceId, product.defaultPrice)}</p>
        </div>
        <button class="w-full h-10 bg-[#137fec] text-white text-sm font-bold rounded-lg hover:bg-blue-600 transition-colors">${getButtonText(product.ctaId, 'Buy Now')}</button>
      </div>
    </div>
  `).join('');
...
<body class="bg-[#f6f7f8] text-[#111418] min-h-screen">
  <!-- Header -->
  <header class="flex items-center justify-between whitespace-nowrap border-b border-solid border-[#f0f2f4] px-4 sm:px-10 lg:px-20 py-3 bg-white">
    <div class="flex items-center gap-8">
      <div class="flex items-center gap-4 text-[#111418]">
        ...
        <h2 class="text-lg font-bold leading-tight tracking-[-0.015em]">${getText('nav_brand', 'MobileShop')}</h2>
      </div>
    </div>
    <div class="flex flex-1 justify-end items-center gap-4">
      <div class="hidden md:flex items-center gap-9">
        <a class="text-sm font-medium leading-normal hover:text-[#137fec]" href="#">${getText('nav_link_1', 'Deals')}</a>
        <a class="text-sm font-medium leading-normal hover:text-[#137fec]" href="#">${getText('nav_link_2', 'New Arrivals')}</a>
        <a class="text-sm font-medium leading-normal hover:text-[#137fec]" href="#">${getText('nav_link_3', 'Best Sellers')}</a>
      </div>
      <div class="flex items-center gap-2">
        <button class="flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 bg-[#f6f7f8] gap-2 text-sm font-bold leading-normal tracking-[0.015em] min-w-0 px-2.5">
        ...
        <div class="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10" style="background-image: url('https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop&crop=face')"></div>
      </div>
    </div>
  </header>

  <!-- Main Content -->
  <main class="px-4 sm:px-6 md:px-10 lg:px-20 xl:px-40 flex flex-1 justify-center py-5">
    <div class="flex flex-col w-full flex-1">
      <!-- Hero Section -->
      <div class="flex flex-wrap justify-between gap-3 p-4">
        <div class="flex w-full flex-col gap-3">
          <h1 class="text-[#111418] text-4xl font-black leading-tight tracking-[-0.033em]">${getText('hero_title', 'Top 50 Best-Selling Mobiles – Big Discounts Live!')}</h1>
          <p class="text-[#617589] text-base font-normal leading-normal">${getText('hero_description', 'Explore our curated list of the best-selling phones with unbeatable discounts.')}</p>
        </div>
      </div>

      <!-- Filter Buttons -->
      <div class="flex gap-3 p-3 overflow-x-auto">
        <button class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-white border border-[#f0f2f4] px-4 hover:bg-gray-50 transition-colors">
          <p class="text-sm font-medium leading-normal">${getText('filter_1', 'Brand')}</p>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="6 9 12 15 18 9"></polyline>
          </svg>
        </button>
        <button class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-white border border-[#f0f2f4] px-4 hover:bg-gray-50 transition-colors">
          <p class="text-sm font-medium leading-normal">${getText('filter_2', 'Price Range')}</p>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="6 9 12 15 18 9"></polyline>
          </svg>
        </button>
        <button class="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-white border border-[#f0f2f4] px-4 hover:bg-gray-50 transition-colors">
          <p class="text-sm font-medium leading-normal">${getText('filter_3', 'Discount')}</p>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="6 9 12 15 18 9"></polyline>
          </svg>
        </button>
      ...
      <div class="flex items-center justify-center p-4">
        <a class="flex size-10 items-center justify-center text-[#111418]" href="#">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="15 18 9 12 15 6"></polyline>
          </svg>
        </a>
        <a class="text-sm font-bold leading-normal tracking-[0.015em] flex size-10 items-center justify-center text-white rounded-full bg-[#137fec]" href="#">1</a>
        <a class="text-sm font-normal leading-normal flex size-10 items-center justify-center rounded-full hover:bg-[#137fec]/20" href="#">2</a>
        <a class="text-sm font-normal leading-normal flex size-10 items-center justify-center rounded-full hover:bg-[#137fec]/20" href="#">3</a>
        <span class="text-sm font-normal leading-normal flex size-10 items-center justify-center rounded-full">...</span>
        <a class="text-sm font-normal leading-normal flex size-10 items-center justify-center rounded-full hover:bg-[#137fec]/20" href="#">8</a>
        <a class="text-sm font-normal leading-normal flex size-10 items-center justify-center rounded-full hover:bg-[#137fec]/20" href="#">9</a>
        <a class="text-sm font-normal leading-normal flex size-10 items-center justify-center rounded-full hover:bg-[#137fec]/20" href="#">10</a>
